#!/system/bin/python
#eaaa bngsd yg mau recode tools gw :v
#Santuy ae gw juga ngerecode tools ini :D

import time, os, sys, time, random, socket, subprocess, httplib

def Logo():
	run()
	subprocess.call("toilet -f standard -F gay Tn.PENNYWISE",shell=True)
...
while(angka<1):
    run()
    mengetik(E+'       PINTU KEMANASAJA.  ')
    mengetik(C+"  Author   : Tn.PENNYWISE")
    mengetik(G+"  My Squad : CORAZON & KEJORA")
    mengetik(E+"  Thanks To: H4NZO, ZxC7, SOLID00, RGS, Mr.Hoax, Marshall.")
    mengetik(E+"             And All member buluq :v ")
    mengetik(E+"  Ketik Website nya  Tanpa http:// !!")
    mengetik(D+"  No Wa gw: 083871259489 ")
    mengetik(I+" Tool ini digunakan hanya untuk pembelajaran :v ")
    mengetik(G+" Penyalahgunaan tool ini diluar tanggungjawab aink ")

    angka+=1
    time.sleep(1)


site00 = raw_input("Masukan Websitenya Boejang :")
